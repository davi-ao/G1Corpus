...
class CommentsSpider(scrapy.Spider):
    name = 'comments'
    start_urls = [
        'http://g1.globo.com/index/feed/pagina-1.html'
    ]
    ...
    def parse_news(self, response):
        link = "http://comentarios.globo.com/comentarios/" + re.search("COMENTARIOS_URI:\s\"[a-z1-9\/-]*\"", response.css('script[id=SETTINGS]').extract()[0]).group(0).encode('ascii')[18:-1].replace('/', '@@') + "/" + re.search("COMENTARIOS_IDEXTERNO:\s\"[a-z0-9\/-]*\"", response.css('script[id=SETTINGS]').extract()[0]).group(0).encode('ascii')[24:-1].replace('/', '@@') + "/" + response.url.replace('/', '@@') + "/shorturl/" + re.search("TITLE:\s\"[^\n]*\"", response.css('script[id=SETTINGS]').extract()[0]).group(0).encode('latin-1')[8:-1] + "/1.json"
